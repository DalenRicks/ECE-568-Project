...
// connect to the server and return the socket fd, or -1 on failure
...
// send a fixed message every second until an error occurs
void send_data(int sock)
{
	...
	while (1)
	{
		...
		if (err < 0)
		{
			ESP_LOGE(TAG, "Send failed, closing connection.");
			break;
		}
